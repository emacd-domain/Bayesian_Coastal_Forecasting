...
load O8_3_visual_crossing_pcs\vc_pcs_2020.mat
...
norm_vc_pcs_2020 = (vc_pcs_2020 - repmat(dm_mean(1:18), [s(1), 1]))./repmat(dm_std(1:18), [s(1), 1]);
norm_era5_pcs_2020 = (era5_pcs_2020 - repmat(dm_mean(1:18), [s(1), 1]))./repmat(dm_std(1:18), [s(1), 1]);

% make adj pcs array
norm_adj_vc_pcs_2020 = zeros(s);

for i=1:s(2)
           
    mdl=fitlm(norm_vc_pcs_2020(:,i), norm_era5_pcs_2020(:, i));
    fc_adj = table2array(mdl.Coefficients(2,1))*norm_vc_pcs_2020(:,i)+table2array(mdl.Coefficients(1,1));
...
pc_slopes=bi';
pc_intercepts=in';
pc_binned_error = 1.5*se;
pc_binned_mean = me;
pc_binned_error_thresholds = pb;
pc_binned_error_thresholds(1,:) = -inf;
pc_binned_error_thresholds(end,:) = inf;

% change labels between u10 and v10
ax = axes(); 
hold on
h(1)=plot(norm_vc_pcs_2020(:, 1), norm_era5_pcs_2020(:, 1),'.b','DisplayName', "Before Correction");
set(gca,'fontname','times','fontsize',8)
ax=gca;
ax.FontSize = 12;
h(2)=plot(norm_adj_vc_pcs_2020(:, 1), norm_era5_pcs_2020(:, 1),'.k','DisplayName',"After Correction");
...
save("pc_binned_error.mat","pc_binned_error")
save("pc_binned_mean.mat","pc_binned_mean")
save("pc_binned_error_thresholds.mat", "pc_binned_error_thresholds")
