...
t=np.round(targets_train[:, -1], decimals=2)
...
while i<9:
    ...
    os.chdir('wave_direction_NS_models')
    ...
    plt.plot([-1,1],[-1.25,0.75],'r:',linewidth=1.5, label='+/- 0.25')
    plt.plot([-1,1],[-0.75,1.25],'r:',linewidth=1.5)
    plt.grid(color='r', linestyle=':', linewidth=0.5)
    plt.xlim([-1.25, 1.25])
    plt.ylim([-1.25, 1.25])
    plt.title("Network "+str(i), fontdict={'fontsize' : 18})
    plt.xlabel("Observed Dm N/S Vector [-]", fontdict={'fontsize' : 18})
    plt.ylabel("Predicted Dm N/S Vector [-]", fontdict={'fontsize' : 18})
    plt.legend()
    ...
    MODELNAME = "FOC_NS_24HOUR_FORECAST_MODEL_"+str(i)
