...
def posterior_target_groups(posttargets, variable_name):
    range_percentiles = percentile(posttargets, arange(0,101,4))
    ...
    for i in range(0, len(PosteriorSamples)):
        ...
        if FitType == 'GaussianMixture':
            for j in tqdm(range(0, Y.shape[1], 1)):
                ...
                if make_plot:
                    ...
                    plt.tight_layout()
                    plt.show()
                    fname=f"GMM of {posterior_variable_list[i]} Posterior Likelihoods for Model {j+1}.png"
        ...
        if FitType == 'Gaussian':
            ...
            for j in tqdm(range(0, Y.shape[1], 1)):
                ...
                ms = exp(gmm.score_samples(xs)) * (max(all_data.reshape(-1,1))/max(exp(gmm.score_samples(xs))))
                #ms = gmm.predict(xs.reshape(-1,1))
                #mss = sum(ms*gmm.weights_/distances, axis=1)
                del gmm
                ...
                if make_plot:
                    ...
                    plt.ylabel("f(x)", fontdict={'fontsize' : 18})
                    plt.xticks(size=14)
                    plt.title(f'GD of {posterior_variable_list[i]} Posterior Likelihoods \n for Model {j+1}')
                    plt.yticks(size=14)
                    plt.tight_layout()
                    plt.show()
                    fname=f"GD of {posterior_variable_list[i]} Posterior Likelihoods for Model {j+1}.png"
                    fig.savefig(plot_directory+'\\'+fname, 
                               dpi=300, format=None, metadata=None,
                               bbox_inches=None, pad_inches=0.1,
                               facecolor='auto', edgecolor='auto',
...
def posterior_evaluation(post_data=None, posterior_variable_list=['Target'], lead_time=24, mod_list=None, estimation_strategy='Interpolation', 
    ...
    elif (estimation_strategy == 'Gaussian'):
        GMList = fit_distribution(PosteriorSamples, idx, posterior_variable_list, PosteriorVariables, VarSortIndexes, post_loop_vars, FitType='Gaussian', make_plot=make_plot, plot_title=plot_title, plot_prefix=plot_prefix, plot_directory=plot_directory)
